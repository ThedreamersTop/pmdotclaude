...
> **IMPORTANT**: Before doing anything else, check the current branch. If on `main`, you MUST create a new branch and switch to it first. Do not skip this step.

1. Run `git branch --show-current` to check the current branch.
   - If on `main`: run `git checkout -b <descriptive-branch-name>` before proceeding.
2. Run `git diff HEAD` to understand what changes have been made
